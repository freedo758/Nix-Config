...

{
  ...
  # This declares ~/.config/cava/config as a Home Manager-managed file
  # (a read-only symlink into the Nix store). That's why editing the
  # `theme` line by hand, or letting matugen try to rewrite this file
  # directly, gets reverted on the next rebuild: Nix always wins.
  #
  # The fix: this file's `theme` value never changes. It always points
  # at ~/.config/cava/themes/matugen — a plain, ordinary file that is
  # NOT managed by Home Manager. matugen is free to overwrite that file
  # at runtime whenever your theme changes, and cava just needs a
  # SIGUSR1 to reload it (see the note below for matugen.nix).
